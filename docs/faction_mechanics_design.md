...
>
> **World Scale-Up (designer directive 2026-08-06, runs FIRST in the substrate phase):** the map keeps its landmass shapes and relations but at substantially higher tile resolution, to accommodate (a) **multi-tile city cores** — small settlements 3 tiles, larger settlements 4, large developed capitals (Empire-class) 7; core only, buildings still placed outside; per-faction footprints differ — and (b) **plentiful scattered Small Ruins** (the permanent interaction layer four factions live on). The pass also **revisits playable-vs-minor faction placement**: playable majors currently sit far from everyone but city-states; use same-culture major↔minor position swaps as the mechanism, driven by an **interlock adjacency matrix** derived from this document (nemesis pairs near each other; Wrath/Gratitude/Corruption/Oath mechanics need neighbors in range; Part 13's separation of the four ruin-hungry factions preserved). **Terrain coherence rule (designer 2026-08-06): at the higher resolution, avoid micro-patches — no 1-2 tile blobs of forest/plains/etc.; enforce a minimum contiguous patch size in terrain generation so the map stays readable and terrain remains gameplay-meaningful.**
